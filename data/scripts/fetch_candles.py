...
from data.scripts.data_config import COIN, RAW_CANDLES_FILE
...
def get_candles(from_timestamp: int, to_timestamp: int) -> Optional[Candles]:
    ohlcv = read_from_cache_or_fetch(RAW_CANDLES_FILE, fetch_candles_from_exchange, f"{COIN}/USDT", from_timestamp,
                                     to_timestamp)
